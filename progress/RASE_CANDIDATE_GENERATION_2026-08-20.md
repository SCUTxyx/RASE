...
- **多样性来源**：同策略候选的多样性完全来自推理随机性。确定性策略（temperature=0 的回归头，如 OFT 的 L1 回归）下，requery 会得到**完全相同的 chunk** → 同策略候选集退化 → 记录 `capability mask`（IDEA §4 规则），不得当普通失败处理。
- **架构差异（调研修正，详见 VLA 候选生成报告）**：
  - **flow matching 系（SmolVLA / π0 / π0.5）**：动作 = 从随机噪声做 10 步 Euler 积分。**没有 temperature/top_p 概念**（那是 VLM 文本解码参数）；"默认动作" = **单次随机采样**（不存在 greedy 等价物）。"默认 vs 候选" = "1 个样本 vs K 个样本"。取 K 候选 = 同一观测批量 K 个噪声（openpi `sample_actions` batch 维 / Octo `sample_shape`），共享 prefix KV cache，最便宜。项目 `select_env_action` 的 temperature 参数即 flow-matching 噪声缩放 `N(0, t²)`（W2 candidate sampling 先例）。
  - **自回归离散 token 系（π0-FAST / OpenVLA / RT-2）**：逐 token 解码，默认 **greedy（temperature=0.0）**；"默认 vs 候选" = "众数 vs 抽样"；取 K 候选 = 温度 0.4–0.7 重复采样（文献仅验证 β=0.7）。
  - **确定性回归系（OpenVLA-OFT）**：单前向输出唯一，**无分布**，同策略候选不可得 → 只能做 `fallback.persistent`。
