...
Status: **frozen after G0; discovery activation permitted**  
G0 artifact SHA256: `aa8cb14ec898093df9b5b93d23e750fcd6f5d131e8af5ac1050c2f1efd77808c`.

This file freezes the execution order and data contracts. G0 completed with
34/36 cells classified B (closed-loop input divergence) and 2/36 classified D;
A and C were absent. This does not claim that discovery, opportunity,
information, or closed-loop gates have passed.
...
steps/cost, latency, and the success-compute Pareto frontier separately. The
frozen primary weights are `lambda_h=1.0`, `lambda_q=0.02`, `lambda_f=0.10`, and
`lambda_l=0.01`. Query and fallback costs are normalized by the 250-step horizon;
latency is normalized by the nominal 25-second 10 Hz episode budget. G-O1's
meaningful minimum effect is 0.03 utility, G-O2 requires eight tasks and three
suites, and the complete sensitivity grid is machine-readable. These values were
fixed after G0 but before discovery outcomes were observed.
...
The frozen discovery verifier chooses the candidate with minimum first-action L2
norm and breaks exact ties by candidate ID; this rule is causal and was selected
before discovery outcomes were observed.
...
- The two decision points are source elapsed steps 8 and 16. Fractions of the
  eventual source trajectory are prohibited because future termination time is
  unavailable to an online controller.
