...
def main() -> int:
    ...
    for i, row in enumerate(rows, start=1):
        ...
        try:
            ...
            continue
        student_chunk = np.asarray(row.get("student_action_chunk", row["action_chunk"]), np.float32).reshape(-1, 7)
        oft_chunk = np.asarray(row["action_chunk"], np.float32).reshape(-1, 7)
