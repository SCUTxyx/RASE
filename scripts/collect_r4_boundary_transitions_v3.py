...
For each PRE-A3 state, follow the persistent OFT trajectory. At frozen handoff
boundaries h={0,8,16,32,64,96,128}, fork the simulator for Student and OFT
counterfactuals, record deterministic SmolVLA latents, and optionally estimate
handback success from K Student continuations of the same saved boundary.
...
def wilson_interval(successes: int, trials: int, *, z: float = 1.6448536269514722) -> tuple[float, float]:
    """One-sided-95%-compatible Wilson interval used for conservative labels."""
    if trials <= 0:
...
def _labels_by_state(
    audit: Mapping[str, Any], *, allow_closed_opportunity_for_eval: bool = False
) -> dict[str, dict[str, Any]]:
    if (audit.get("safe_handback_status") != "ready"
            and not allow_closed_opportunity_for_eval):
        raise ValueError(
            "safe-handback gate is closed: " + repr(audit.get("safe_handback_reasons"))
        )
    if audit.get("true_task_join_status") != "ready":
        raise ValueError("true task-ID join is not certified")
    if audit.get("deterministic_prefix_consistency_status") not in {None, "ready"}:
        raise ValueError("deterministic OFT-prefix consistency is not certified")
    labels = {}
...
def repeat_seeds_for_boundary(
    *, state_key: str, elapsed: int, repeats: int, shared_seed: int,
    paired_across_boundaries: bool, rollout_seed_fn: Any,
) -> list[int]:
    """Return frozen continuation seeds, optionally paired across boundaries."""
    if paired_across_boundaries:
        return [
            int(rollout_seed_fn(state_key, 0, repeat_index, salt=2_026_080_911))
            for repeat_index in range(repeats)
        ]
    return [int(shared_seed)] + [
        int(rollout_seed_fn(state_key, elapsed, repeat_index, salt=2_026_080_409))
...
def collect_state(
    *,
    ...
    boundaries: list[int],
    chunk_steps: int,
    handback_repeats: int,
    paired_repeat_seeds_across_boundaries: bool,
    ...
    try:
        ...
        while not terminal:
            ...
            if elapsed in boundary_set:
                ...
                repeat_seeds = repeat_seeds_for_boundary(
                    state_key=state_key,
                    elapsed=elapsed,
                    repeats=handback_repeats,
                    shared_seed=shared_seed,
                    paired_across_boundaries=paired_repeat_seeds_across_boundaries,
                    rollout_seed_fn=rollout_seed,
                )
                handback_rollouts = [
                    rollout_student_handback(
                ...
                handback_successes = [
                    bool(rollout["success"]) for rollout in handback_rollouts
                ]
                handback_success_count = sum(handback_successes)
                handback_success_probability = handback_success_count / handback_repeats
                handback_lcb, handback_ucb = wilson_interval(
                    handback_success_count, handback_repeats
                )
                conservative_handback_success = all(handback_successes)
                ...
                rows.append(
                    {
                        "schema_version": "rase-pre-c0-r5-probabilistic-boundary-transition/v1",
                        "state_key": state_key,
                        ...
                        "success_if_handback_now": conservative_handback_success,
                        "handback_binary_target_definition": "all_repeats_succeeded",
                        "handback_repeats": handback_repeats,
                        "handback_repeat_seeds": repeat_seeds,
                        "handback_repeat_seed_pairing": (
                            "common_across_boundaries"
                            if paired_repeat_seeds_across_boundaries
                            else "legacy_first_seed_only"
                        ),
                        "handback_repeat_successes": handback_successes,
                        "handback_repeat_steps": [
                            int(rollout["steps"]) for rollout in handback_rollouts
                        ],
                        "handback_repeat_stop_reasons": [
                            str(rollout["stop_reason"]) for rollout in handback_rollouts
                        ],
                        "handback_success_count": handback_success_count,
                        "handback_success_probability": handback_success_probability,
                        "handback_success_wilson_lcb_95_one_sided": handback_lcb,
                        "handback_success_wilson_ucb_95_one_sided": handback_ucb,
                        "handback_continuation_steps": int(handback_rollouts[0]["steps"]),
                        "handback_stop_reason": str(handback_rollouts[0]["stop_reason"]),
                        "historical_success_if_handback_now": historical_handback_success,
                        "success_if_continue_oft": None,
                        "student_failure_risk": not conservative_handback_success,
                        "student_failure_probability": 1.0 - handback_success_probability,
        ...
        return {
            "schema_version": "rase-pre-c0-r5-probabilistic-state-transitions/v1",
            "state_key": state_key,
            "task_id": str(label["task_id"]),
...
def main() -> int:
    ...
    parser.add_argument(
        "--handback-repeats", type=int, default=1,
        help="Student continuations per exact saved boundary snapshot",
    )
    parser.add_argument(
        "--paired-repeat-seeds-across-boundaries", action="store_true",
        help="Use the same repeat-seed set at every boundary of a state.",
    )
    ...
    parser.add_argument(
        "--allow-closed-opportunity-for-eval",
        action="store_true",
        help=("Allow a frozen val/test split to be collected even when its small-sample "
              "opportunity gate is not ready. Forbidden for train; task join and prefix "
    ...
    if args.handback_repeats < 1:
        raise ValueError("handback-repeats must be positive")
    ...
    if args.allow_closed_opportunity_for_eval and args.split not in {"val", "test"}:
        raise ValueError("closed-opportunity bypass is allowed only for val/test")
    labels = _labels_by_state(
        audit,
        allow_closed_opportunity_for_eval=args.allow_closed_opportunity_for_eval,
    )
    state_meta = {
    ...
    requested_in_suite: set[str] = set()
    for state_key, meta in state_meta.items():
        ...
        if _suite(pool_meta.task_id) == args.suite:
            requested_in_suite.add(state_key)
            selected.append(state_key)
    selected.sort()
    # Validate the suite/split selection before sharding.  Comparing all
    # requested suite states against one shard incorrectly reports every state
    # assigned to a sibling shard as missing when num_shards > 1.
    missing_requested = sorted(requested_in_suite - set(selected))
    ...
    for index, state_key in enumerate(selected, start=1):
        ...
        if target.is_file() and not args.force:
            ...
            result = collect_state(
                ...
                handback_repeats=args.handback_repeats,
                paired_repeat_seeds_across_boundaries=(
                    args.paired_repeat_seeds_across_boundaries
                ),
                adapter=adapter,
                libero_plus_root=libero_plus_root,
            )
    ...
    report = {
        "schema_version": "rase-pre-c0-r5-probabilistic-boundary-collection/v1",
        "suite": args.suite,
        ...
        "handback_repeat_seed_pairing": (
            "common_across_boundaries"
            if args.paired_repeat_seeds_across_boundaries
            else "legacy_first_seed_only"
        ),
        "handback_binary_target_definition": "all_repeats_succeeded",
        ...
        "source_safe_handback_status": audit.get("safe_handback_status"),
        "closed_opportunity_bypassed_for_evaluation": bool(
            args.allow_closed_opportunity_for_eval
            and audit.get("safe_handback_status") != "ready"
        ),
