...
CONFIG="${CONFIG:-configs/collect_rase_ui_phase0b_opportunity12.json}"
TAG="${TAG:-screen_v1}"
FRESH_RUN="${FRESH_RUN:-1}"
POOL="runs/rase_ui_phase0b_opportunity12_pool"
KEYS="runs/rase_ui_phase0b_opportunity12_keys.json"
SMOL_RUN="runs/rase_ui_phase0b_opportunity12_smol_${TAG}"
OFT_PREFIX="rase_ui_phase0b_opportunity12_oft"
MATRIX="runs/rase_ui_phase0b_opportunity12_matrix_${TAG}"
LOG="runs/rase_ui_phase0b_opportunity12_${TAG}.log"
...
if [[ "$FRESH_RUN" == "1" ]]; then
  targets=("$POOL" "$KEYS" "$SMOL_RUN" "$MATRIX")
...
echo "=== PHASE0B PREFLIGHT ==="
"$PY" scripts/preflight_runner.py --min-free-gpu-mib 20000

echo "=== PHASE0B COLLECT 12 EPISODES ==="
if [[ "$FRESH_RUN" == "0" && -f "$POOL/manifest.json" ]]; then
  ...
  "$PY" scripts/collect_state_pool.py \
    --config "$CONFIG" \
    --summary-output "runs/rase_ui_phase0b_opportunity12_collection_summary.json"
fi

echo "=== PHASE0B FREEZE STRICT-CONTINUE KEYS ==="
...
echo "=== PHASE0B STRICT CONTINUE / REPLAN ==="
smol_mode=(--resume)
if [[ "$FRESH_RUN" == "1" ]]; then
  smol_mode=(--fresh-run)
fi
...
echo "=== PHASE0B SWITCH_POLICY(OFT), SUITE-SERIAL ==="
...
echo "=== PHASE0B ASSEMBLE THREE-OPERATOR MATRIX ==="
assemble_mode=()
if [[ "$FRESH_RUN" == "1" ]]; then
...
echo "=== PHASE0B SUCCESS-ONLY OPPORTUNITY GATE ==="
set +e
...
echo "=== PHASE0B STRATIFIED DIAGNOSTICS ==="
"$PY" scripts/analyze_intervention_matrix.py \
  --matrix-dir "$MATRIX" \
  --output "$MATRIX/analysis.json" \
  --bootstrap-replicates 10000

echo "=== PHASE0B EXPLORATORY COST SENSITIVITY ==="
...
echo "PHASE0B_DONE matrix=$MATRIX audit_status=$audit_status"
