...
def aggregate_confirmation(rows: list[dict], *, repeats: int, weights: dict) -> list[dict]:
    ...
    by_root_operator: dict[tuple[str, str], list[dict]] = defaultdict(list)
    for row in point_values:
        by_root_operator[(row["root_id"], row["operator_id"])].append(row)
    result = []
    for (root_id, operator_id), values in sorted(by_root_operator.items()):
        exemplar = values[0]
        result.append({
            "root_id": root_id, "operator_id": operator_id,
            "policy_id": exemplar["policy_id"], "task_id": exemplar["task_id"],
...
def _gap(rows: list[dict]) -> float:
    ...
    oracle = np.mean([max(item["utility"] for item in values) for values in by_root.values()])
