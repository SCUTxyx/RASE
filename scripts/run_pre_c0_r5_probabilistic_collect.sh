...
ALLOW_CLOSED_OPPORTUNITY_FOR_EVAL="${ALLOW_CLOSED_OPPORTUNITY_FOR_EVAL:-0}"
HANDBACK_REPEATS="${HANDBACK_REPEATS:-1}"
PAIRED_REPEAT_SEEDS="${PAIRED_REPEAT_SEEDS:-0}"
...
if ! [[ "$HANDBACK_REPEATS" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR: HANDBACK_REPEATS must be a positive integer" >&2
  exit 1
fi
if [[ "$PAIRED_REPEAT_SEEDS" != "0" && "$PAIRED_REPEAT_SEEDS" != "1" ]]; then
  echo "ERROR: PAIRED_REPEAT_SEEDS must be 0 or 1" >&2
  exit 1
fi
if [[ -n "$TARGET_MANIFEST" ]]; then
  if [[ -n "$TARGET_STATE" || -n "$TARGET_STATES" ]]; then
    echo "ERROR: TARGET_MANIFEST cannot be combined with TARGET_STATE/TARGET_STATES" >&2
    exit 1
  fi
  if [[ ! -f "$TARGET_MANIFEST" ]]; then
    echo "ERROR: TARGET_MANIFEST does not exist: $TARGET_MANIFEST" >&2
    exit 1
  fi
  TARGET_STATES="$($BASE_PY - "$TARGET_MANIFEST" <<'PY'
import json, sys
payload = json.load(open(sys.argv[1]))
records = payload.get("records") or []
if not records:
    raise SystemExit("target manifest contains no records")
print(",".join(str(row["state_key"]) for row in records))
PY
)"
fi
"$BASE_PY" - "$AUDIT" "$SPLIT_FILTER" "$ALLOW_CLOSED_OPPORTUNITY_FOR_EVAL" <<'PY'
import json, sys
report = json.load(open(sys.argv[1]))
split = sys.argv[2]
allow_eval = sys.argv[3] == "1"
if allow_eval and split not in {"val", "test"}:
    raise SystemExit("closed-opportunity bypass is forbidden for train")
if report.get("safe_handback_status") != "ready" and not allow_eval:
...
if report.get("deterministic_prefix_consistency_status") not in {None, "ready"}:
    raise SystemExit("deterministic OFT-prefix consistency is not certified")
PY
...
for idx in "${!SUITE_SHORTS[@]}"; do
  ...
  if [[ -n "$TARGET_STATES" ]]; then
    IFS=',' read -r -a target_values <<< "$TARGET_STATES"
    for target in "${target_values[@]}"; do
      if [[ -z "$target" ]]; then
        echo "ERROR: TARGET_STATES contains an empty state key" >&2
        exit 1
      fi
      target_args+=(--state-key "$target")
    done
  fi
  boundary_args=()
  if [[ -n "$BOUNDARIES" ]]; then
    IFS=',' read -r -a boundary_values <<< "$BOUNDARIES"
    for boundary in "${boundary_values[@]}"; do
      if ! [[ "$boundary" =~ ^[0-9]+$ ]]; then
        echo "ERROR: BOUNDARIES must be comma-separated non-negative integers" >&2
        exit 1
      fi
      boundary_args+=(--boundary "$boundary")
    done
  fi
  eval_gate_args=()
  if [[ "$ALLOW_CLOSED_OPPORTUNITY_FOR_EVAL" == "1" ]]; then
    eval_gate_args+=(--allow-closed-opportunity-for-eval)
  fi
  paired_seed_args=()
  if [[ "$PAIRED_REPEAT_SEEDS" == "1" ]]; then
    paired_seed_args+=(--paired-repeat-seeds-across-boundaries)
  fi
  worker_pids=()
  for worker in $(seq 0 $((PARALLEL_WORKERS - 1))); do
    (
      ...
      python -u scripts/collect_r4_boundary_transitions_v3.py \
        --config "$CONFIG" \
        ...
        --handback-repeats "$HANDBACK_REPEATS" \
...
merged = {
    **{key: reports[0][key] for key in (
        ...
        "handback_repeats", "handback_binary_target_definition",
        ...
        "source_safe_handback_status", "closed_opportunity_bypassed_for_evaluation",
...
out.write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in rows))
summary = {
    "schema_version": "rase-pre-c0-r5-probabilistic-boundary-merged/v1",
    "n_suites": len(reports),
    "n_states": sum(r["n_states"] for r in reports),
    "n_rows": len(rows),
    "persistent_replay_matches": sum(r["persistent_replay_matches"] for r in reports),
    "projection_sha256": next(iter(projection_hashes)),
    "handback_repeats": reports[0]["handback_repeats"],
    "handback_binary_target_definition": reports[0]["handback_binary_target_definition"],
    "handback_repeat_seed_pairing": reports[0]["handback_repeat_seed_pairing"],
