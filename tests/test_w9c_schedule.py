...
from rase.collect.w9c_schedule import (
    BATCH_SIZES,
    PROTOCOL_VERSION,
    generate_w9c_schedule,
    load_w9c_schedule,
    requests_for_batch,
    schedule_bytes,
    schedule_sha256,
    validate_w9c_schedule,
    write_w9c_schedule,
)


def test_schedule_is_byte_deterministic_and_seed_sensitive():
    first = generate_w9c_schedule(20260731)
    second = generate_w9c_schedule(20260731)
    changed = generate_w9c_schedule(20260732)
...
def test_schedule_batches_are_balanced_and_do_not_reset_init_allocation():
    payload = generate_w9c_schedule(20260731)
...
def test_task_and_init_use_independent_salted_mappings():
    rows = validate_w9c_schedule(generate_w9c_schedule(20260731))
...
def test_scheduled_init_is_required_and_bounds_checked():
    request = requests_for_batch(generate_w9c_schedule(20260731), 1)[0]
...
def _w9c_test_config(tmp_path: Path) -> tuple[dict, Path]:
    schedule_path = tmp_path / "schedule.json"
    payload = generate_w9c_schedule(20260731)
    digest = write_w9c_schedule(schedule_path, payload)
    output = tmp_path / "runs" / "ngc_w9c_resume_smoke"
    config = {
        "run_name": "ngc-w9c-test-smoke",
        "protocol": {
...
def test_resume_uses_same_episode_task_and_init_without_duplicates(tmp_path):
    config, output = _w9c_test_config(tmp_path)
    ...
    rows = requests_for_batch(generate_w9c_schedule(20260731), 1)
    expected = {
...
def test_w9c_config_cannot_write_legacy_w9_pool(tmp_path):
    config, _ = _w9c_test_config(tmp_path)
    config["collection"]["output_dir"] = "pool/ngc_w9_clean_controls"
    config["collection"]["smoke_mode"] = False
    path = tmp_path / "config.json"
    path.write_text(json.dumps(config), encoding="utf-8")
    with pytest.raises(ValueError, match="W9C output_dir|legacy W9"):
        load_config(path)
