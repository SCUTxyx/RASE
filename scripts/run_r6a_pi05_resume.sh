...
# Preserve the completed post-fix smoke when resuming after an intentional
# pause; otherwise run it once before any formal Pi0.5 outcomes.
if [[ ! -s "$ROOT/smoke/pi05_seed0_v2/summary.json" ]]; then
  $PY scripts/rollout_lerobot_source_from_initial_states.py \
    ...
    --max-states 1 \
    --fresh-run \
    --output-dir "$ROOT/smoke/pi05_seed0_v2"
fi
