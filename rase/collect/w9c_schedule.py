"""Frozen deterministic W9C clean-control episode schedule."""
...
SCHEDULE_SCHEMA = "rase-w9c-clean-control-schedule/v1"
PROTOCOL_VERSION = "W9C-clean-control/v1"
...
def _balanced_suites(seed: int, batch_id: int, size: int) -> list[str]:
    if size % len(SUITES):
        raise ValueError("W9C batch sizes must divide evenly across suites")
...
def generate_w9c_schedule(seed: int) -> dict[str, Any]:
    """Generate all 140 frozen rows with independent salted mappings."""
    rows: list[W9CScheduleRow] = []
        ...
        for request_index, suite in enumerate(suites):
            ...
            rows.append(
                W9CScheduleRow(
                    ...
                    perturb_sub="none",
                    level=0,
                    episode_id=f"ep-w9c-{global_index:08d}",
                )
            )
            global_index += 1
    payload = {
...
def write_w9c_schedule(path: Path, payload: Mapping[str, Any]) -> str:
    validate_w9c_schedule(payload)
...
def _row_from_mapping(value: Mapping[str, Any]) -> W9CScheduleRow:
    try:
        return W9CScheduleRow(
        ...
        raise ValueError(f"invalid W9C schedule row: {value}") from exc


def validate_w9c_schedule(payload: Mapping[str, Any]) -> list[W9CScheduleRow]:
    if payload.get("schema_version") != SCHEDULE_SCHEMA:
        raise ValueError("unsupported W9C schedule schema")
    if payload.get("protocol_version") != PROTOCOL_VERSION:
        raise ValueError("W9C protocol version mismatch")
    if tuple(payload.get("batch_sizes") or ()) != BATCH_SIZES:
        raise ValueError("W9C batch sizes must be exactly 60/40/40")
    if int(payload.get("max_episodes", -1)) != MAX_EPISODES:
        raise ValueError("W9C maximum must be exactly 140 episodes")
    if int(payload.get("n_init_states_per_task", -1)) != DEFAULT_N_INIT_STATES:
        raise ValueError("W9C schedule must target exactly 50 init states per task")
    raw_rows = payload.get("rows")
    if not isinstance(raw_rows, Sequence) or isinstance(raw_rows, (str, bytes)):
        raise ValueError("W9C schedule rows must be an array")
    rows = [_row_from_mapping(row) for row in raw_rows]
    if len(rows) != MAX_EPISODES:
        raise ValueError(f"W9C schedule requires {MAX_EPISODES} rows")
    if [row.global_episode_index for row in rows] != list(range(MAX_EPISODES)):
        raise ValueError("W9C schedule has missing or reordered global rows")
    episode_ids = [row.episode_id for row in rows]
    if len(set(episode_ids)) != len(episode_ids):
        raise ValueError("W9C schedule has duplicate episode_id values")

    by_batch: dict[int, list[W9CScheduleRow]] = defaultdict(list)
    seen_inits: dict[tuple[str, int], list[int]] = defaultdict(list)
    for row in rows:
        if row.suite not in SUITES:
            raise ValueError(f"invalid W9C suite {row.suite!r}")
        if row.task_id not in range(1, N_CLEAN_TASKS + 1):
            raise ValueError(f"W9C task_id out of range: {row.task_id}")
        if row.init_state_id not in range(DEFAULT_N_INIT_STATES):
            raise ValueError(f"W9C init_state_id out of range: {row.init_state_id}")
        if (
            row.perturb_dim != "clean"
            or row.perturb_sub != "none"
            or row.level != 0
        ):
            raise ValueError("W9C rows must use clean/none/L0 semantics")
    ...
    for batch_id, expected_size in enumerate(BATCH_SIZES, 1):
        ...
        if len(batch) != expected_size:
            raise ValueError(f"W9C batch {batch_id} requires {expected_size} rows")
        if [row.request_index for row in batch] != list(range(expected_size)):
            raise ValueError(f"W9C batch {batch_id} request indices are not contiguous")
        suite_counts = {suite: sum(row.suite == suite for row in batch) for suite in SUITES}
        if len(set(suite_counts.values())) != 1:
            raise ValueError(f"W9C batch {batch_id} is not suite-balanced")
    ...
    for task_key, init_ids in seen_inits.items():
        for start in range(0, len(init_ids), DEFAULT_N_INIT_STATES):
            ...
            if len(cycle) != len(set(cycle)):
                raise ValueError(
                    f"W9C init_state_id repeats before exhaustion for {task_key}"
                )
    return rows


def load_w9c_schedule(path: Path, *, expected_sha256: str) -> dict[str, Any]:
    raw = path.read_bytes()
    actual = hashlib.sha256(raw).hexdigest()
    if actual != expected_sha256:
        raise ValueError(
            f"W9C schedule SHA256 mismatch: expected {expected_sha256}, got {actual}"
        )
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError(f"invalid W9C schedule JSON: {exc}") from exc
    validate_w9c_schedule(payload)
    if raw != schedule_bytes(payload):
        raise ValueError("W9C schedule is not canonical byte-stable JSON")
    return payload
...
def requests_for_batch(
    ...
    rows = validate_w9c_schedule(payload)
    if batch_id not in range(1, len(BATCH_SIZES) + 1):
        raise ValueError("W9C batch_id must be 1, 2, or 3")
