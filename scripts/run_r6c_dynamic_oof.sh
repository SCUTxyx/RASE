#!/usr/bin/env bash
# R6-C no-world-model dynamic-risk baseline: five frozen task-held-out OOF
# training seeds, per-VLA stage gate.  Gated: run only after the R6-B1.2
# collection passes the source-parity audit.
set -euo pipefail
cd /root/autodl-tmp/RASE

PYTHON_BIN="${PYTHON_BIN:-/root/autodl-tmp/envs/smolvla/bin/python}"
DATASET_ROOT="${DATASET_ROOT:-runs/pre_c0_r6/r6b1_b1p2_v1}"
OUTPUT_ROOT="${OUTPUT_ROOT:-runs/pre_c0_r6/r6c_dynamic_oof_v1}"
DATASET="${DATASET:-$DATASET_ROOT/r6c_dynamic_dataset.npz}"
DATASET_REPORT="${DATASET_REPORT:-$DATASET_ROOT/r6c_dynamic_dataset.npz.report.json}"
...
# Build the frozen dataset once from the gated B1.2 collection.
if [[ ! -f "$DATASET" ]]; then
  "$PYTHON_BIN" scripts/build_r6c_dynamic_dataset.py \
    --input-root "$DATASET_ROOT" \
    --protocol "$PROTOCOL" \
    --output "$DATASET"
fi
if [[ "$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["status"])' "$DATASET_REPORT")" != "complete" ]]; then
  echo "R6C ERROR: dataset report is not complete" >&2
  exit 2
fi
...
for seed in $SEEDS; do
  ...
  for policy in pi0fast_libero pi05_libero; do
    "$PYTHON_BIN" scripts/train_r6c_dynamic_risk_oof.py \
      --dataset "$DATASET" \
