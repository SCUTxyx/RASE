#!/usr/bin/env bash
# Suite-by-suite long-horizon successful OFT teacher collection for PRE-C1.1.
set -euo pipefail
...
for idx in "${!SUITE_SHORTS[@]}"; do
  ...
  mkdir -p "$suite_out"
  echo "=== PRE-C1.1 OFT server suite=${suite} horizon_steps=${HORIZON_STEPS} (0=persistent) ==="
  kill_server
  (
    conda activate oft
    ...
    exec python -m rase.oracle.server --endpoint "$ENDPOINT" \
      ...
      > "runs/oft_server_pre_c1_1_${short}.log" 2>&1
  ) &
  ready=0
  ...
  if [[ "$ready" != "1" ]]; then
    ...
    tail -50 "runs/oft_server_pre_c1_1_${short}.log" >&2 || true
    exit 1
  fi
  limit_args=()
  if [[ "$LIMIT" != "0" ]]; then
  ...
  "$PY" scripts/collect_pre_c1_1_oft_success_chunks.py \
    --config "$CONFIG" \
    --rollout-dir "$ROLLOUT" \
    --suite "$label" \
    --endpoint "$ENDPOINT" \
    ...
    --resume \
    "${limit_args[@]}"
  # Flatten successful trajectory JSONs to TEACHER_DIR root for indexing.
  for f in "$suite_out"/*.json; do
    [[ -f "$f" ]] || continue
    cp -f "$f" "$TEACHER_DIR"/
  done
  # Preserve chunk dirs under suite output; also symlink under TEACHER_DIR if needed.
