...
OUT=runs/pre_c0_r6/r6b1_smoke_pi0fast_spatial_parity_v4
mkdir -p "$OUT"

CUDA_VISIBLE_DEVICES=0 \
PYTHONPATH="/root/autodl-tmp/src/openvla-oft:$PWD" \
RASE_OFT_CHECKPOINT="$PWD/ckpts/oft_spatial" \
RASE_OFT_SUITE=libero_spatial \
"$OFT_PY" -m rase.oracle.server --endpoint "$ENDPOINT" \
  --adapter rase.oracle.openvla_oft_adapter:create_adapter \
  > "$OUT/oft_server.log" 2>&1 &
server_pid=$!
cleanup() {
...
for _ in $(seq 1 60); do
  if "$VLA_PY" scripts/probe_oracle.py --endpoint "$ENDPOINT" --expect-suite libero_spatial >/dev/null 2>&1; then
    ready=1
    break
  fi
  sleep 5
